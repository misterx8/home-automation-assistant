...
> Versione architettura: 5.7 (RK-59 fix: ping via zwave_js.ping invece di MQTT zwave2mqtt; RK-58 broadcast immediato disarmo; RK-57 default_entity_id; plancia_controllo_generale)
...
| RK-59 | **OSSERVABILITÀ** — Nessun feedback se un comando LED non viene ricevuto dalla tastiera (Z-Wave è fire-and-forget a livello applicativo): un pacchetto perso o un nodo temporaneamente irraggiungibile causava desync silenzioso senza alcun log. | **Implementato 2026-04-20 — Corretto 2026-04-21** | Aggiunto `input_text.ring_keypad_zwave_entity_ids` (JSON `{topic_base: entity_id_zwave_js}`) in `globals.yaml` — sostituisce i vecchi `zwave_gateway`+`node_ids` (che usavano MQTT interno di zwave2mqtt, non il modo corretto). Nuovo script `ring_keypad_ping_verify(keypad_id)` (`mode: queued`): 2s delay → `zwave_js.ping` con `response_variable` → controlla `success` nella risposta → se false logga `🚨 PING FALLITO`. Nuovo script `ring_keypad_verify_all` (`mode: restart`): itera tastiere sequenzialmente. `ring_keypad_sync_all` chiama `verify_all` fire-and-forget dopo il loop sync. |
